...
class SlackZipArchive {
    ...
    async process() {
        ...
        // Load the users file
        this.userDataFile = await this.zip.file(`${this.zipName}/users.json`).async('string');
        ...
        }).map(folderName => folderName.split('/')[1])
    ...
    async toZip(outputDir) {
        ...
        .then(function(content) {
            ...
            const outputPath = path.join(outputDir, `output-${startDateFormatted}-${endDateFormatted}.zip`);
