...
class SlackChannel {
    ...
    constructor(parentArchive, channelName, skipFiles, filterStartDate, filterEndDate) {
    ...
    async process() {
        ...
        for (let zipTranscriptFile of this.zipTranscriptFiles) {
            ...
            for (let message of transcriptJson) {
                ...
                // If a date range is specified we'll only keep files that belong to messages in that range
                let keepFiles = true; 
                if (this.filterStartDate && this.filterEndDate) {
                    const messageDate = new Date(message.ts * 1000);
                    keepFiles = messageDate >= this.filterStartDate && messageDate <= this.filterEndDate
                }

                if (message.files && keepFiles) {
        ...
        // If a filter date range has been provided filte down to just the messages that are within those dates inclusive
        if (this.filterStartDate && this.filterEndDate) {
            this.messages = this.messages.filter(message => message.date >= this.filterStartDate && message.date <= this.filterEndDate);
        }
    ...
    static async fromJsZip(parentArchive, channelName, skipFiles, filterStartDate, filterEndDate) {

        const slackChannel = new SlackChannel(parentArchive, channelName, skipFiles, filterStartDate, filterEndDate);
